...
//! Specifically, we use the [Noise IK][ik] handshake which is a one round-trip
//! protocol (the client sends one message, then the server responds).
//! For more information about Noise and our implementation, refer to the
//! [crypto] crate.
//!
//! Usage example:
//!
...
//! };
//! use aptos_crypto::{ed25519, test_utils::TEST_SEED, x25519, PrivateKey, Uniform};
...
//!     let client_context =
//!         NetworkContext::new(RoleType::Validator, NetworkId::Validator, client_peer_id);
...
//!     let server_context =
//!         NetworkContext::new(RoleType::Validator, NetworkId::Validator, server_peer_id);
...
//!         client_session.map_err(|err| io::Error::new(io::ErrorKind::Other, err))?;
//!     let (mut server_session, _client_peer_id, _trust_level) =
//!         server_session.map_err(|err| io::Error::new(io::ErrorKind::Other, err))?;
//!
//!     // client -> server
