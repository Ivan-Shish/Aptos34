...
/// To reduce lock contention, the container locks only at the granularity of
/// a tuple (network_id, peer_id). This container is updated by both the
/// networking code (e.g., for new peer connections and lost peer connections),
/// as well as individual applications (e.g., peer monitoring service).
#[derive(Debug)]
pub struct PeersAndMetadata {
    peers_and_metadata: HashMap<NetworkId, RwLock<HashMap<PeerId, PeerMetadata>>>,

    // We cache the connected peers and metadata as applications are
    // expected to poll these quite frequently. We also cache per
    // network and protocol to allow more granular lookups.
    _connected_peer_metadata: HashMap<(NetworkId, ProtocolId), Vec<(PeerNetworkId, PeerMetadata)>>,
}

impl PeersAndMetadata {
    /// Create a new `PeersAndMetadata` container.
    pub fn new(network_ids: &[NetworkId]) -> Arc<PeersAndMetadata> {
        ...
        let mut peers_and_metadata = PeersAndMetadata {
            ...
            _connected_peer_metadata: HashMap::new(),
        };
    ...
    pub fn get_networks(&self) -> impl Iterator<Item = NetworkId> + '_ {
