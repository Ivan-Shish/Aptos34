...
/// An [`AndThen`] is a transport which applies a closure (F) to all connections
/// created by the underlying transport.
...
/// Listener stream returned by [listen_on](Transport::listen_on) on an AndThen
/// transport.
#[pin_project]
#[derive(Debug)]
...
///
/// Takes a future (Fut1) generated from an underlying transport, runs it to
/// completion and applies a closure (F) to the result to create another future
/// (Fut2) which is then run to completion.
#[pin_project]
#[derive(Debug)]
