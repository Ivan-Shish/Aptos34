...
/// Gauge for tracking the number of in-flight pings
pub static IN_FLIGHT_PINGS: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "peer_monitoring_client_in_flight_pings",
        "Gauge related to the number of in-flight pings",
        &["network"]
    )
    .unwrap()
});

/// Updates the metrics for the number of in-flight pings
pub fn update_in_flight_pings(label: &str, num_in_flight_pings: u64) {
    set_gauge(&IN_FLIGHT_PINGS, label, num_in_flight_pings);
}
