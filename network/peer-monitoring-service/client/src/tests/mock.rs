...
impl MockMonitoringServer {
    ...
    pub fn add_peer_to_peers_and_metadata(
        &mut self,
    ...
    pub fn disconnect_peer_from_peers_and_metadata(&mut self, peer: PeerNetworkId) {
        self.update_peer_state(peer, ConnectionState::Disconnected);
    }

    /// Reconnects the peer in the peers and metadata struct
    pub fn reconnect_peer_to_peers_and_metadata(&mut self, peer: PeerNetworkId) {
    ...
    pub async fn next_request(&mut self, network_id: &NetworkId) -> Option<NetworkRequest> {
        ...
        let peer_manager_request_receiver = self
            .peer_manager_request_receivers
            .get_mut(network_id)
            .unwrap();
