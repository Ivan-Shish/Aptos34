...
impl PerformanceMonitoringState {
    pub fn new(
        ...
        // Verify the performance monitoring config is valid
        verify_performance_monitoring_config(&performance_monitoring_config);

        // Calculate the request interval
        let request_interval_usecs = if performance_monitoring_config.enable_direct_send_testing {
        ...
        let request_tracker = RequestTracker::new(request_interval_usecs, time_service);
...
impl StateValueInterface for PerformanceMonitoringState {
    ...
    fn get_request_timeout_ms(&self) -> u64 {
        // Verify the request is not a direct send request
        if self
            .performance_monitoring_config
            .enable_direct_send_testing
        {
            panic!("Direct send requests do not support timeouts!");
        } else {
        }

        // Return the RPC response timeout
    ...
    fn handle_monitoring_service_response(
        ...
        let monitoring_service_request = match monitoring_service_request {
            ...
            request => {
                error!(LogSchema::new(LogEntry::PerformanceMonitoringRequest)
...
/// Verifies the performance monitoring config is valid
fn verify_performance_monitoring_config(
    performance_monitoring_config: &PerformanceMonitoringConfig,
) {
    let direct_send_testing = performance_monitoring_config.enable_direct_send_testing;
    let rpc_testing = performance_monitoring_config.enable_rpc_testing;

    // Verify that exactly one testing mode is enabled
    if direct_send_testing && rpc_testing {
        panic!(
            "Cannot enable both direct send and RPC testing! Only one testing mode is supported."
        );
    }
    ...
    if !(performance_monitoring_config.enable_direct_send_testing
        || performance_monitoring_config.enable_rpc_testing)
    {
        panic!("Either direct send or RPC testing must be enabled!");
    }
}
...
mod test {
    ...
    fn test_verify_performance_monitoring_state() {
        ...
        assert!(performance_monitoring_state.request_data.is_none());
    ...
    fn handle_monitoring_service_response(
        ...
        let peer_monitoring_service_response =
            PeerMonitoringServiceResponse::PerformanceMonitoringResponse(
                PerformanceMonitoringResponse { response_counter },
            );
