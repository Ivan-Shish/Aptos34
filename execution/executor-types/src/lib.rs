...
pub trait ChunkExecutorTrait: Send + Sync {
    ...
    /// If the transactions are valid, executes them and returns the
    /// executed result for commit.
    fn execute_chunk(
        &self,
    ...
    /// Similar to `execute_chunk`, but instead of executing transactions, apply
    /// the transaction outputs directly to get the executed result.
    fn apply_chunk(
        &self,
    ...
    /// Finishes the chunk executor by releasing memory held by inner data
    /// structures(SMT).
    fn finish(&self);
}
...
pub trait BlockExecutorTrait: Send + Sync {
    ...
    /// Finishes the block executor by releasing memory held by inner data
    /// structures(SMT).
    fn finish(&self);
}
...
/// A structure that holds relevant information about a chunk that was
/// committed.
...
pub struct StateComputeResult {
    ...
    /// accumulator after the execution. For details, please see
    /// [`InMemoryAccumulator`](aptos_types::proof::accumulator::InMemoryAccumulator).
...
impl StateComputeResult {
    ...
    /// this function is used in RandomComputeResultStateComputer to assert that
    /// the compute function is really called.
    ...
    /// generate a new dummy state compute result with
    /// ACCUMULATOR_PLACEHOLDER_HASH as the root hash. this function is used
    /// in ordering_state_computer as a dummy state compute result,
    /// where the real compute result is generated after
    /// ordering_state_computer.commit pushes the blocks and the finality
    /// proof to the execution phase.
...
pub struct TransactionData {
    /// Each entry in this map represents the new value of a store store object
