...
impl FakeExecutor {
    ...
    fn handle_account_creation(
        ...
        let mut new_account: Account = Default::default();
        new_account.authentication_key = new_account_address.to_vec();

        let mut new_coin_store: CoinStore = Default::default();
        new_coin_store.coin = account_creation_info.initial_balance;

        let mut total_supply: u128 =
            Self::get_value(&TOTAL_SUPPLY_STATE_KEY, &state_view)?.unwrap();
