...
Navigate to the fungible asset move module examples directory:

```bash
cd ~/aptos-core/aptos-move/move-examples/fungible_asset
```

There are multiple files as different examples in the same package. For this tutorial, actually only `managed_fungible_asset.move` and `multisig_managed_coin.move` are necessary. The other files could be deleted before publishing the package.

Then publish this package onto your `default` account using CLI:
