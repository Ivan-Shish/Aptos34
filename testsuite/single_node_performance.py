...
def execute_command(command):
    result = []
    with Popen(command, shell=True, text=True, cwd=target_directory, stdout=PIPE, bufsize=1, universal_newlines=True) as p:
...
with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    for (transaction_type, use_native_executor, module_working_set_size), (
        ...
        for threads_per_counter in [1, 2, 4, 8]:
            ...
            common_command_suffix = f"{use_native_executor_str} --threads-per-counter {threads_per_counter} --generate-then-execute --transactions-per-sender 1 --block-size {cur_block_size} --use-state-kv-db --use-sharded-state-merkle-db run-executor --transaction-type {transaction_type} --module-working-set-size {module_working_set_size} --main-signer-accounts {MAIN_SIGNER_ACCOUNTS} --additional-dst-pool-accounts {ADDITIONAL_DST_POOL_ACCOUNTS} --data-dir {tmpdirname}/db  --checkpoint-dir {tmpdirname}/cp"
            for concurrency_level in EXECUTION_ONLY_CONCURRENCY_LEVELS:
                ...
                # )
                gps = float(
                    re.findall(r"Overall fraction in VM: (\d+\.?\d*) fraction", output)[-1]
                )
            ...
            print(
                tabulate(
                    gas_rows,
                    headers=["transaction_type", "module_working_set", "executor", "threads_per_counter", "block_size"]
                    + [
                        f"exe_only {concurrency_level}"
                        for concurrency_level in EXECUTION_ONLY_CONCURRENCY_LEVELS
                    ]
