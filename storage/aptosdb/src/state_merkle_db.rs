    ...
    pub fn get_state_snapshot_version_before(
        ...
        if next_version > 0 {
            ...
            if let Some((key, _node)) = iter.next().transpose()? {
                // TODO: If we break up a single update batch to multiple commits, we would need
                // to deal with a partial version, which hasn't got the root
                // committed.
...
impl TreeReader<StateKey> for StateMerkleDb {
    ...
    fn get_rightmost_leaf(&self, version: Version) -> Result<Option<(NodeKey, LeafNode)>> {
        // Since everything has the same version during restore, we seek to the first
        // node and get its version.
        let mut iter = self.iter::<JellyfishMerkleNodeSchema>(Default::default())?;
        iter.seek(&(version, 0))?;
        ...
        //
        // Here version is fixed. For each num_nibbles, there could be a range of nibble
        // paths of the same length. If one of them is the rightmost leaf R, it
        // must be at the end of this range. Otherwise let's assume the R is in
        // the middle of the range, so we call the node at the end of this range
        // X:   1. If X is leaf, then X.account_key() > R.account_key(), because
        // the nibble path is a      prefix of the account key. So R is not the
        // rightmost leaf.   2. If X is internal node, then X must be on the
        // right side of R, so all its children's      account keys are larger
        // than R.account_key(). So R is not the rightmost leaf.
        //
        // Given that num_nibbles ranges from 0 to ROOT_NIBBLE_HEIGHT, there are only
        // ROOT_NIBBLE_HEIGHT+1 ranges, so we can just find the node at the end of each
        // range and then pick the one with the largest account key.
        let mut ret = None;

        for num_nibbles in 1..=ROOT_NIBBLE_HEIGHT + 1 {
            let mut iter = self.iter::<JellyfishMerkleNodeSchema>(Default::default())?;
            // nibble_path is always non-empty except for the root, so if we use an empty
            // nibble path as the seek key, the iterator will end up pointing to
