...
pub trait DBPruner: Send + Sync {
    ...
    /// Returns the target version for the current pruning round - this might be
    /// different from the target_version() because we need to keep
    /// max_version in account.
    ...
    /// Records the current progress of the pruner by updating the least
    /// readable version
