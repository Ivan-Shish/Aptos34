    ...
    K: crate::Key + CryptoHash + 'static,
{
    pub fn new<D: 'static + TreeReader<K> + TreeWriter<K>>(
            ...
            tree_reader.get_node_option(&NodeKey::new_empty_path(version))?
        {
            ...
            // If the system crashed in the middle of the previous restoration attempt, we
            // need to recover the partial nodes to the state right before the
            // crash.
            (
                false,
    ...
    fn recover_partial_nodes(
        ...
        // Start from the parent of the rightmost leaf. If this internal node exists in
        // storage, it is not a partial node. Go to the parent node and repeat
        // until we see a node that does not exist. This node and all its
        // ancestors will be the partial nodes.
        ...
        // Next we reconstruct all the partial nodes up to the root node, starting from
        // the bottom. For all of them, we scan all its possible child positions
        // and see if there is one at each position. If the node is not the
        // bottom one, there is additionally a partial node child at the
        // position `previous_child_index`.
        let mut partial_nodes = vec![];
        // Initialize `previous_child_index` to `None` for the first iteration of the
        // loop so the code below treats it differently.
        ...
        loop {
            ...
            // partial node and we do not know its hash yet. For the lowest partial node, we
            // just find all its known children from storage in the loop above.
    ...
    fn insert_at_leaf(
        ...
        // The node at this position becomes an internal node. Since we may insert more
        // nodes at this position in the future, we do not know its hash yet.
        ...
        // Next we build the new internal nodes from top to bottom. All these internal
        // node except the bottom one will now have a single internal node
        // child.
        ...
        // Do not set the new child for now. We always call `freeze` first, then set the
        // new child later, because this way it's easier in `freeze` to find the
        // correct leaf to freeze -- it's always the rightmost leaf on the
        // lowest level.
        self.partial_nodes.push(internal_info);
    ...
    fn freeze_internal_nodes(&mut self, num_remaining_nodes: usize) {
        while self.partial_nodes.len() > num_remaining_nodes {
            ...
            // Keep the hash of this node before moving it into `frozen_nodes`, so we can
            // update its parent later.
            ...
            // Now that we have computed the hash of the internal node above, we will also
            // update its parent unless it is root node.
            if let Some(parent_node) = self.partial_nodes.last_mut() {
                ...
                match parent_node.children[rightmost_child_index] {
                    ...
                    _ => {
                        panic!(
                            "Must have at least one child and the rightmost child must not be a \
                             leaf."
                        )
                    },
                }
    ...
    fn verify(&self, proof: SparseMerkleRangeProof) -> Result<()> {
        ...
        // If we have all siblings on the path from root to `previous_key`, we should be
        // able to compute the root hash. The siblings on the right are already
        // in the proof. Now we compute the siblings on the left side, which
        // represent all the accounts that have ever been added.
        let mut left_siblings = vec![];

        // The following process might add some extra placeholder siblings on the left,
        // but it is nontrivial to determine when the loop should stop. So
        // instead we just add these siblings for now and get rid of them in the
        // next step.
        ...
        // Now we remove any extra placeholder siblings at the bottom. We keep removing
        // the last sibling if 1) it's a placeholder 2) it's a sibling on the
        // left.
    ...
    pub fn finish_impl(mut self) -> Result<()> {
        self.wait_for_async_commit()?;
        // Deal with the special case when the entire tree has a single leaf or null
        // node.
