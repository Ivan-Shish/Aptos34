    ...
    H: CryptoHasher,
{
    ...
    /// smaller accumulator.
    ///
    /// See [`aptos_types::proof::AccumulatorConsistencyProof`] for proof
    /// format.
    ...
    H: CryptoHasher,
{
    ...
    fn append(&self, new_leaves: &[HashValue]) -> Result<(HashValue, Vec<Node>)> {
        ...
        // Iterate over the new leaves, adding them to to_freeze and then adding any
        // frozen parents when right children are encountered.  This has the
        // effect of creating frozen nodes in perfect post-order, which can be
        // used as a strictly increasing append only index for the underlying
        // storage.
        //
        // We will track newly created left siblings while iterating so we can pair them
        // with their right sibling, if and when it becomes frozen.  If the
        // frozen left sibling is not created in this iteration, it must already
        // exist in storage.
        ...
        for (leaf_offset, leaf) in new_leaves.iter().enumerate() {
            ...
            // The node remaining must be a left child, possibly the root of a complete
            // binary tree.
            left_siblings.push((pos, hash));
        }

        // Now reconstruct the final root hash by walking up to root level and adding
        // placeholder hash nodes as needed on the right, and left siblings that have
        // either been newly created or read from storage.
