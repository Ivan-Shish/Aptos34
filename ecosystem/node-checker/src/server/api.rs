...
// I choose to keep both methods rather than making these two separate APIs
// because it'll make for more descriptive error messages. We write the function
// comment on one line because the OpenAPI generator does some wonky newline
// stuff otherwise. Currently Poem doesn't support "flattening" a struct into
// separate query parameters, so I do that myself. See https://github.com/poem-web/poem/issues/241.
#[OpenApi]
impl<R: Runner> Api<R> {
    ...
    /// without including any port or endpoints. All other parameters are
    /// optional. For example, if your node's API port is open but the rest
    /// are closed, only set the `api_port`.
    #[oai(path = "/check", method = "get")]
    async fn check(
        &self,
        /// The ID of the baseline node configuration to use for the evaluation,
        /// e.g. devnet_fullnode
        baseline_configuration_id: Query<String>,
        /// The URL of the node to check, e.g. http://44.238.19.217 or http://fullnode.mysite.com
        node_url: Query<Url>,
        /// If given, we will assume the metrics service is available at the
        /// given port.
        metrics_port: Query<Option<u16>>,
        /// If given, we will assume the API is available at the given port.
        api_port: Query<Option<u16>>,
        /// If given, we will assume that clients can communicate with your node
        /// via noise at the given port.
        noise_port: Query<Option<u16>>,
        /// A public key for the node, e.g.
