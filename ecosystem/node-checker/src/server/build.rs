...
/// is a wrapper around the baseline configuration and its Runner, which
/// contains all the Checkers, Providers, etc. we built based on the
/// configuration.
///
/// In this function, we attempt to build Providers that operate against the
/// baseline node. If the configuration specifies some information that means we
/// can build a Provider, e.g. they provide a noise port, we will try to build
/// that Provider, in this example a NoiseProvider. If we can't build it / the
/// `provide` call fails, we throw an error that will ultimately result in
/// startup failure. In short, we choose to fail at startup if a baseline
/// Provider that should be working is not working.
async fn build_baseline_configuration_runner(
    ...
    if let Some(node_address) = &configuration.node_address {
        ...
        if let Some(metrics_client) = metrics_client {
            ...
            metrics_provider.provide().await.context(format!(
                "Failed to build MetricsProvider for baseline configuration {}, ensure the \
            ...
            system_information_provider
                ...
                .await
                .context(format!(
                    "Failed to build SystemInformationProvider for baseline configuration {}, \
                     ensure the /system_information endpoint is accessible",
                    configuration.configuration_id
                ))?;
            info!(
        ...
        if let Some(api_client) = api_client {
            ...
            api_index_provider.provide().await.context(format!(
                "Failed to build ApiProvider for baseline configuration {}, ensure the API is \
                 accessible",
                configuration.configuration_id
            ))?;
            info!(
            ...
            // If we have an ApiIndexProvider and a noise port / public key, we can try to
            // create a NoiseProvider.
            if node_address.get_noise_port().is_some() || node_address.get_public_key().is_some() {
                let noise_network_address =
                    node_address.as_noise_network_address().context(format!(
                        "Failed to build NoiseProvider for baseline configuration {}, ensure the \
                         noise port is accessible",
                        configuration.configuration_id
                    ))?;
                ...
                // Specifically check that we can establish a connection with the baseline node
                // on the noise port.
                noise_provider
                    .establish_connection()
                    .await
                    .context(format!(
                        "Failed to build NoiseProvider for baseline configuration {}, ensure a \
                         noise port and public key were provided and they're both valid",
                        configuration.configuration_id
                    ))?;
    ...
    if let Some(node_address) = &configuration.node_address {
        let message = format!(
            "Failed to run the Checker suite against the baseline node itself for {}. This \
             implies that a Checker has been enabled in the baseline config but the necessary \
             baseline information was not provided. For example, this error might happen if the \
             NodeIdentityChecker was enabled but the API port for the baseline was not provided, \
             since that checker needs to be able to query the API of the baseline node. ",
            configuration.configuration_id
        );
        let results = runner
            .run(node_address)
