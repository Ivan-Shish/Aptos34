...
//!
//! The structs and functions in this module together form a simplified *model*
//! of how common Aptos transactions should behave. This model is then used as
//! an *oracle* for property-based tests -- the results of executing
//! transactions through the VM should match the results computed using
//! this model.
//!
...
/// `UNIVERSE_SIZE` environment variable.
///
/// Larger values will provide greater testing but will take longer to run and
/// shrink. Release mode is recommended for values above 100.
#[inline]
// REVIEW: this was changed from pub(crate) to pub in order to separate the
// actual tests from the e2e tests library. Take a closer look and decide what
// we really want to do:   1. pub(crate) -> pub
...
/// The number of transactions to run universe-based proptests with. Set with
/// the `UNIVERSE_SIZE` environment variable (this function will return twice
/// that).
///
/// Larger values will provide greater testing but will take longer to run and
/// shrink. Release mode is recommended for values above 100.
...
pub trait AUTransactionGen: fmt::Debug {
    ...
    /// on the VM and the expected values: the transaction status and the
    /// gas used.
    fn apply(
        &self,
...
/// Represents the current state of account in a universe, possibly after its
/// state has been updated by running transactions against the universe.
...
impl AccountCurrent {
    ...
    /// Returns the current balance for this account, assuming all transactions
    /// seen so far are applied.
    pub fn balance(&self) -> u64 {
        self.balance
    }

    /// Returns the current sequence number for this account, assuming all
    /// transactions seen so far are applied.
    pub fn sequence_number(&self) -> u64 {
        self.sequence_number
    }

    /// Returns the current sent events count for this account, assuming all
    /// transactions seen so far are applied.
    pub fn sent_events_count(&self) -> u64 {
        self.sent_events_count
    }

    /// Returns the current received events count for this account, assuming all
    /// transactions seen so far are applied.
    ...
    /// Returns the gas cost of a create-account transaction where the sender
    ...
    /// Returns the gas cost of a create-account transaction where the account
    /// exists already.
    ...
    /// Returns the gas cost of a peer-to-peer transaction with an insufficient
    /// balance.
...
/// Computes the result for running a transfer out of one account. Also updates
/// the account to reflect this transaction.
///
/// The return value is a pair of the expected status and whether the
...
pub fn txn_one_account_result(
    ...
    match (enough_max_gas, enough_to_transfer, enough_to_succeed) {
        ...
        (true, true, false) => {
            ...
            // succeed in the epilogue. The transaction will be run and gas will
            // be deducted from the sender, but no other changes will happen.
            sender.sequence_number += 1;
...
/// Run these transactions and make sure that they all cost the same amount of
/// gas.
...
/// Verify that the account information in the universe matches the information
/// in the executor.
pub fn assert_accounts_match(
    ...
    for (idx, account) in universe.accounts().iter().enumerate() {
        ...
        // XXX These two don't work at the moment because the VM doesn't bump up event
        // counts.        prop_assert_eq!(
