...
/// The gas each transaction is configured to reserve. If the gas available in
/// the account, converted to microaptos, falls below this threshold,
/// transactions are expected to fail with an insufficient balance.
pub const TXN_RESERVED: u64 = 500_000;

/// The gas cost of a first time create-account transaction.
///
/// This includes the cost of the event counter creation which makes the
/// transaction more expensive. All such transactions are expected to cost the
/// same gas.
...
/// The gas cost of a create-account transaction where the sender has an
/// insufficient balance.
///
/// This includes the cost of the event counter creation. As such the cost of
/// the transaction would be higher and the balance required must be higher.
/// All such transactions are expected to cost the same gas.
pub static CREATE_ACCOUNT_TOO_LOW_FIRST: Lazy<u64> = Lazy::new(|| {
    let mut executor = FakeExecutor::from_head_genesis();
    // The gas amount is the minimum that needs to be reserved, so use a value
    // that's clearly higher than that.
...
/// The gas cost of a create-account transaction where the receiver already
/// exists.
///
/// This includes the cost of the event counter creation. As such the cost of
/// the transaction would be higher and the balance required must be higher.
...
/// The gas cost of a peer-to-peer transaction that tries to create a new
/// account, but fails because of an insufficient balance.
///
/// This includes the cost of the event counter creation. As such the cost of
/// the transaction would be higher and the balance required must be higher.
/// All such transactions are expected to cost the same gas.
