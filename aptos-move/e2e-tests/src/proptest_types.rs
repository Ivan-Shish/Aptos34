...
impl Arbitrary for Account {
    ...
    fn arbitrary_with(_params: ()) -> Self::Strategy {
        // Provide Account::new as the canonical strategy. This means that no shrinking
        // will happen, but that's fine as accounts have nothing to shrink
        // inside them anyway.
...
impl AccountData {
    ...
    pub fn strategy(balance_strategy: impl Strategy<Value = u64>) -> impl Strategy<Value = Self> {
        // Pick sequence numbers and event counts in a smaller range so that valid
