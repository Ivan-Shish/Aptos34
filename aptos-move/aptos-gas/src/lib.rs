...
//!   - Sets the initial values for all gas parameters, including the
//!     instruction, transaction move-stdlib and aptos-framework ones.
//!   - Defines a bi-directional mapping between the (Rust) gas parameter
//!     structs and their corresponding representation on-chain.
//!
//! The reason why we need two different representations is that they serve
//! different purposes:
//!   - The Rust structs are used for quick (static) lookups by the gas meter
//!     and native functions when calculating gas costs.
//!   - The on-chain gas schedule needs to be extensible and unordered so we can
//!     upgrate it easily in the future.
