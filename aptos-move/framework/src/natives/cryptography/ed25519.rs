...
fn native_public_key_validate(
    ...
    // This deserialization only performs point-on-curve checks, so we check for
    // small subgroup below NOTE(Gas): O(1) cost: some arithmetic for converting
    // to (X, Y, Z, T) coordinates
    ...
    // Check if the point lies on a small subgroup. This is required when using
    // curves with a small cofactor (e.g., in Ed25519, cofactor = 8).
...
///                            +? ( per_sig_strict_verify_cost +
/// per_msg_hashing_base_cost                                 +
...
fn native_signature_verify_strict(
    ...
    // NOTE(Gas): hashing the message to the group and a size-2 multi-scalar
    // multiplication
...
pub struct GasParameters {
    ...
    pub per_msg_byte_hashing: InternalGasPerByte, /* signature verification involves signing
                                                   * |msg| bytes */
}
