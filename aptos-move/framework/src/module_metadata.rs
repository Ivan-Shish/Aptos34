...
/// The keys used to identify the metadata in the metadata section of the module
/// bytecode. This is more or less arbitrary, besides we should use some unique
/// key to identify Aptos specific metadata (`aptos::` here).
...
pub struct RuntimeModuleMetadata {
    /// The error map containing the description of error reasons as grabbed
    /// from the source. These are typically only a few entries so no
    /// relevant size difference.
    pub error_map: BTreeMap<u64, ErrorDescription>,
}

/// V1 of Aptos specific metadata attached to the metadata section of
/// file_format.
...
/// Extract metadata from a compiled module, upgrading V0 to V1 representation
/// as needed.
