...
pub enum EntryFunctionCall {
    ...
    /// Mint more token from an existing token_data. Mint only adds more token
    /// to property_version 0
    TokenMintScript {
    ...
    /// if the token property_version is 0, we will create a new
    /// property_version per token to generate a new token_id per token
    /// if the token property_version is not 0, we will just update the
    /// propertyMap and use the existing token_id (property_version)
...
/// Mint more token from an existing token_data. Mint only adds more token to
/// property_version 0
...
/// mutate the token property and save the new property in TokenStore
/// if the token property_version is 0, we will create a new property_version
/// per token to generate a new token_id per token if the token property_version
/// is not 0, we will just update the propertyMap and use the existing token_id
/// (property_version)
