...
// (`ScriptCall`) and the standard BCS-compatible representation used in Aptos
// transactions (`Script`).
//
// This code was generated by compiling known Script interfaces ("ABIs") with
// the tool `aptos-sdk-builder`.
...
pub enum EntryFunctionCall {
    /// Offers signer capability on behalf of `account` to the account at
    /// address `recipient_address`. An account can delegate its signer
    /// capability to only one other address at one time.
    /// `signer_capability_key_bytes` is the
    /// `SignerCapabilityOfferProofChallengeV2` signed by the account owner's
    /// key `account_scheme` is the scheme of the account (ed25519 or
    /// multi_ed25519). `account_public_key_bytes` is the public key of the
    /// account owner. `recipient_address` is the address of the recipient
    /// of the signer capability - note that if there's an existing
    /// `recipient_address` in the account owner's `SignerCapabilityOffer`, this
    /// will replace the previous `recipient_address` upon successful
    /// verification (the previous recipient will no longer have access
    ...
    /// Revoke the account owner's signer capability offer for
    /// `to_be_revoked_address` (i.e., the address that has a signer
    /// capability offer from `account` but will be revoked in this function).
    AccountRevokeSignerCapability {
        to_be_revoked_address: AccountAddress,
    },

    /// Generic authentication key rotation function that allows the user to
    /// rotate their authentication key from any scheme to any scheme.
    /// To authorize the rotation, we need two signatures:
    /// - the first signature `cap_rotate_key` refers to the signature by the
    ///   account owner's current key on a valid `RotationProofChallenge`,
    /// demonstrating that the user intends to and has the capability to rotate
    /// the authentication key of this account;
    /// - the second signature `cap_update_table` refers to the signature by the
    ///   new key (that the account owner wants to rotate to) on a
    /// valid `RotationProofChallenge`, demonstrating that the user owns the new
    /// private key, and has the authority to update the
    /// `OriginatingAddress` map with the new address mapping <new_address,
    /// originating_address>. To verify signatures, we need their
    /// corresponding public key and public key scheme: we use `from_scheme` and
    /// `from_public_key_bytes` to verify `cap_rotate_key`, and `to_scheme`
    /// and `to_public_key_bytes` to verify `cap_update_table`. A scheme of
    /// 0 refers to an Ed25519 key and a scheme of 1 refers to Multi-Ed25519
    /// keys. `originating address` refers to an account's original/first
    /// address.
    ...
    /// Set whether `account` can receive direct transfers of coins that they
    /// have not explicitly registered to receive.
    AptosAccountSetAllowDirectCoinTransfers {
        allow: bool,
    },

    /// Convenient function to transfer APT to a recipient account that might
    /// not exist. This would create the recipient account first, which also
    /// registers it to receive APT, before transferring.
    AptosAccountTransfer {
        to: AccountAddress,
        amount: u64,
    },

    /// Convenient function to transfer a custom CoinType to a recipient account
    /// that might not exist. This would create the recipient account first
    /// and register it to receive the CoinType, before transferring.
    ...
    /// Only callable in tests and testnets where the core resources account
    /// exists. Claim the delegated mint capability and destroy the
    /// delegated token.
    AptosCoinClaimMintCapability {},

    /// Only callable in tests and testnets where the core resources account
    /// exists. Create delegated token for the address so the account could
    /// claim MintCapability later.
    AptosCoinDelegateMintCapability {
        to: AccountAddress,
    },

    /// Only callable in tests and testnets where the core resources account
    /// exists. Create new coins and deposit them into dst_addr's account.
    AptosCoinMint {
    ...
    /// @param execution_hash Required. This is the hash of the resolution
    /// script. When the proposal is resolved, only the exact script with
    /// matching hash can be successfully executed.
    ...
    /// Create a single-step or multi-step proposal with the backing
    /// `stake_pool`. @param execution_hash Required. This is the hash of
    /// the resolution script. When the proposal is resolved, only the exact
    /// script with matching hash can be successfully executed.
    ...
    /// Same as `publish_package` but as an entry function which can be called
    /// as a transaction. Because of current restrictions for txn
    /// parameters, the metadata needs to be passed in serialized form.
    ...
    /// Mint and Burn Capabilities will be stored under `account` in
    ...
    /// withdraw and deposit event handlers. Required if user wants to start
    /// accepting deposits of `CoinType` in his account.
    ManagedCoinRegister {
        coin_type: TypeTag,
    },

    /// Creates a new resource account and rotates the authentication key to
    /// either the optional auth key if it is non-empty (though auth keys
    /// are 32-bytes) or the source accounts current auth key.
    ResourceAccountCreateResourceAccount {
        seed: Vec<u8>,
        optional_auth_key: Vec<u8>,
    },

    /// Creates a new resource account, transfer the amount of coins from the
    /// origin to the resource account, and rotates the authentication key
    /// to either the optional auth key if it is non-empty (though auth keys
    /// are 32-bytes) or the source accounts current auth key. Note,
    /// this function adds additional resource ownership to the resource account
    /// and should only be used for resource accounts that need access to
    /// Coin<AptosCoin>.
    ...
    /// Creates a new resource account, publishes the package under this account
    /// transaction under this account and leaves the signer cap readily
    ...
    /// Similar to increase_lockup_with_cap but will use ownership capability
    /// from the signing account.
    StakeIncreaseLockup {},

    /// Initialize the validator account and give ownership to the signing
    /// account except it leaves the ValidatorConfig to be set by another
    /// entity. Note: this triggers setting the operator and owner, set it
    /// to the account's address to set later.
    ...
    /// Request to have `pool_address` leave the validator set. The validator is
    /// only actually removed from the set when the next epoch starts.
    /// The last validator in the set cannot leave. This is an edge case that
    /// should never happen as long as the network is still operational.
    ///
    ...
    /// Rotate the consensus key of the validator, it'll take effect in next
    /// epoch.
    ...
    /// Similar to unlock_with_cap but will use ownership capability from the
    /// signing account.
    StakeUnlock {
        amount: u64,
    },

    /// Update the network and full node addresses of the validator. This only
    ...
    /// Staker can call this function to create a simple staking contract with a
    /// specified operator.
    ...
    /// Allow anyone to distribute already unlocked funds. This does not affect
    /// reward compounding and therefore does not need to be restricted to
    /// just the staker or operator.
    StakingContractDistribute {
        staker: AccountAddress,
        operator: AccountAddress,
    },

    /// Unlock commission amount from the stake pool. Operator needs to wait for
    /// the amount to become withdrawable at the end of the stake pool's
    /// lockup period before they can actually can withdraw_commission.
    ///
    ...
    /// Convenient function to allow the staker to reset their stake pool's
    /// lockup period to start now.
    StakingContractResetLockup {
        operator: AccountAddress,
    },

    /// Allows staker to switch operator without going through the lenghthy
    /// process to unstake.
    ...
    /// Allows staker to switch operator without going through the lenghthy
    /// process to unstake, without resetting commission.
    ...
    /// Staker can call this to request withdrawal of part or all of their
    /// staking_contract. This also triggers paying commission to the
    /// operator for accounting simplicity.
    StakingContractUnlockStake {
        operator: AccountAddress,
        amount: u64,
    },

    /// Convenient function to allow the staker to update the voter address in a
    /// staking contract they made.
    ...
    /// Remove the beneficiary for the given shareholder. All distributions will
    /// sent directly to the shareholder account.
    ...
    /// Terminate the vesting contract and send all funds back to the withdrawal
    /// address.
...
impl EntryFunctionCall {
    /// Build an Aptos `TransactionPayload` from a structured object
    /// `EntryFunctionCall`.
    pub fn encode(self) -> TransactionPayload {
    ...
    /// Try to recognize an Aptos `TransactionPayload` and convert it into a
    /// structured object `EntryFunctionCall`.
...
/// Offers signer capability on behalf of `account` to the account at address
/// `recipient_address`. An account can delegate its signer capability to only
/// one other address at one time. `signer_capability_key_bytes` is the
/// `SignerCapabilityOfferProofChallengeV2` signed by the account owner's key
...
/// `recipient_address` is the address of the recipient of the signer capability
/// - note that if there's an existing `recipient_address` in the account
/// owner's `SignerCapabilityOffer`, this will replace the
/// previous `recipient_address` upon successful verification (the previous
/// recipient will no longer have access to the account owner's signer
/// capability).
...
/// Revoke the account owner's signer capability offer for
/// `to_be_revoked_address` (i.e., the address that has a signer capability
/// offer from `account` but will be revoked in this function).
...
/// Generic authentication key rotation function that allows the user to rotate
/// their authentication key from any scheme to any scheme. To authorize the
/// rotation, we need two signatures:
/// - the first signature `cap_rotate_key` refers to the signature by the
///   account owner's current key on a valid `RotationProofChallenge`,
/// demonstrating that the user intends to and has the capability to rotate the
/// authentication key of this account;
/// - the second signature `cap_update_table` refers to the signature by the new
///   key (that the account owner wants to rotate to) on a
/// valid `RotationProofChallenge`, demonstrating that the user owns the new
/// private key, and has the authority to update the `OriginatingAddress` map
/// with the new address mapping <new_address, originating_address>.
/// To verify signatures, we need their corresponding public key and public key
/// scheme: we use `from_scheme` and `from_public_key_bytes`
/// to verify `cap_rotate_key`, and `to_scheme` and `to_public_key_bytes` to
/// verify `cap_update_table`. A scheme of 0 refers to an Ed25519 key and a
/// scheme of 1 refers to Multi-Ed25519 keys. `originating address` refers to an
/// account's original/first address.
...
/// Set whether `account` can receive direct transfers of coins that they have
/// not explicitly registered to receive.
...
/// Convenient function to transfer APT to a recipient account that might not
/// exist. This would create the recipient account first, which also registers
/// it to receive APT, before transferring.
...
/// Convenient function to transfer a custom CoinType to a recipient account
/// that might not exist. This would create the recipient account first and
/// register it to receive the CoinType, before transferring.
...
/// Only callable in tests and testnets where the core resources account exists.
/// Create delegated token for the address so the account could claim
/// MintCapability later.
...
/// When the proposal is resolved, only the exact script with matching hash can
...
/// Same as `publish_package` but as an entry function which can be called as a
/// transaction. Because of current restrictions for txn parameters, the
...
/// Initialize new coin `CoinType` in Aptos Blockchain.
/// Mint and Burn Capabilities will be stored under `account` in `Capabilities`
/// resource.
...
/// Creates a new resource account, transfer the amount of coins from the origin
/// to the resource account, and rotates the authentication key to either the
/// optional auth key if it is non-empty (though auth keys are 32-bytes) or the
/// source accounts current auth key. Note, this function adds additional
/// resource ownership to the resource account and should only be
...
/// Creates a new resource account, publishes the package under this account
/// transaction under this account and leaves the signer cap readily available
/// for pickup.
...
/// Similar to increase_lockup_with_cap but will use ownership capability from
/// the signing account.
...
/// Note: this triggers setting the operator and owner, set it to the account's
/// address to set later.
...
/// Request to have `pool_address` leave the validator set. The validator is
/// only actually removed from the set when the next epoch starts.
/// The last validator in the set cannot leave. This is an edge case that should
/// never happen as long as the network is still operational.
///
/// Can only be called by the operator of the validator/staking pool.
...
/// Update the network and full node addresses of the validator. This only takes
...
/// Allow anyone to distribute already unlocked funds. This does not affect
/// reward compounding and therefore does not need to be restricted to just the
/// staker or operator.
...
/// Unlock commission amount from the stake pool. Operator needs to wait for the
/// amount to become withdrawable at the end of the stake pool's lockup period
/// before they can actually can withdraw_commission.
///
/// Only staker or operator can call this.
...
/// Convenient function to allow the staker to reset their stake pool's lockup
/// period to start now.
...
/// Allows staker to switch operator without going through the lenghthy process
/// to unstake.
...
/// Allows staker to switch operator without going through the lenghthy process
/// to unstake, without resetting commission.
...
/// Staker can call this to request withdrawal of part or all of their
/// staking_contract. This also triggers paying commission to the operator for
...
/// Withdraw all funds to the preset vesting contract's withdrawal address. This
/// can only be called if the contract has already been terminated.
