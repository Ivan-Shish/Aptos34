...
use crate::{
    ...
    task::{DataView, ModulePath, Transaction},
...
pub struct VersionedView<'a, T: Transaction, D: DataView<T = T> + ?Sized> {
    base_view: &'a D,
    versioned_view: ViewMapKind<'a, T>,
    txn_idx: TxnIndex,
}

impl<'a, T: Transaction, D: DataView<T = T> + ?Sized> VersionedView<'a, T, D> {
    pub fn new_mv_view(
        base_view: &'a D,
...
impl<'a, T: Transaction, D: DataView<T = T> + ?Sized> DataView for VersionedView<'a, T, D> {
    type T = T;
