...
module token_objects::piggy_bank {
    use std::error;
    use std::option;
    ...
    const COLLECTION_NAME: vector<u8> = b"Piggy Bank Collection Name";
    /// The ambassador token collection description
    const COLLECTION_DESCRIPTION: vector<u8> = b"Piggy Bank Collection Description";
    /// The ambassador token collection URI
    const COLLECTION_URI: vector<u8> = b"Piggy Bank Collection URI";
    ...
    struct PiggyBankBalance has key {
        piggy_bank_balance: u64,
    }

    /// The ambassador level update event
    struct BalanceUpdateEvent has drop, store {
        old_balance: u64,
        new_balance: u64,
    }
    ...
    fun init_module(sender: &signer) {
        create_piggy_bank_collection(sender);
    }
    ...
    fun create_piggy_bank_collection(creator: &signer) {
    ...
    public entry fun mint_piggy_bank_token(
        creator: &signer,
        description: String,
        name: String,
        uri: String,
