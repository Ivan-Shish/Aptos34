...
impl AptosVM {
    ...
    /// Get the paranoid type check flag if already set, otherwise return
    /// default true
    ...
    /// Returns the # of async proof reading threads if already set, otherwise
    /// return default value (32).
    ...
    /// Generates a transaction output for a transaction that encountered errors
    /// during the execution process. This is public for now only for tests.
    ...
    /// TODO: this is going to be deprecated and removed in favor of code
    /// publishing via NativeCodeContext
    ...
    /// Resolve a pending code publish request registered via the
    /// NativeCodeContext.
    ...
    pub(crate) fn execute_user_transaction<S: MoveResolverExt + StateView>(
        ...
        if self.0.get_gas_feature_version() >= 1 {
            ...
            // This is to ensure we correctly charge for loading certain resources, even if
            // they have been previously cached in the prologue.
            //
            // TODO(Gas): Do this in a better way in the future, perhaps without forcing the
            // data cache to be flushed.
    ...
    fn read_writeset(
        ...
        // All Move executions satisfy the read-before-write property. Thus we need to
        // read each access path that the write set is going to update.
...
impl VMExecutor for AptosVM {
    /// Execute a block of `transactions`. The output vector will have the exact
    /// same length as the input vector. The discarded transactions will be
    /// marked as `TransactionStatus::Discard` and have an empty `WriteSet`.
    /// Also `state_view` is immutable, and does not have interior
    /// mutability. Writes to be applied to the data view are encoded in the
    /// write set part of a transaction output.
    fn execute_block(
...
impl VMValidator for AptosVM {
    /// Determine if a transaction is valid. Will return `None` if the
    /// transaction is accepted, `Some(Err)` if the VM rejects it, with
    /// `Err` as an error code. Verification performs the following steps:
    /// 1. The signature on the `SignedTransaction` matches the public key
    /// included in the    transaction
    /// 2. The script to be executed is under given specific configuration.
    /// 3. Invokes `Account.prologue`, which checks properties such as the
    /// transaction has the right sequence number and the sender has enough
    /// balance to pay for the gas. TBD:
    /// 1. Transaction arguments matches the main function's type signature.
    ///    We don't check this item for now and would execute the check at
    /// execution time.
...
impl AptosSimulationVM {
    ...
    fn simulate_signed_transaction<S: MoveResolverExt + StateView>(
        ...
        // malicious fullnodes may execute them without user's explicit
        // permission.
