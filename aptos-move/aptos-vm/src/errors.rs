...
/// Error codes that can be emitted by the prologue. These have special
/// significance to the VM when they are raised during the prologue.
/// These errors are only expected from the module that is registered as the
/// account module for the system. The prologue should not emit any other error
/// codes or fail for any reason, doing so will result in the VM throwing an
/// invariant violation
...
/// Converts particular Move abort codes to specific validation error codes for
/// the prologue Any non-abort non-execution code is considered an invariant
/// violation, specifically `UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION`
        ...
        VMStatus::MoveAbort(location, code) => {
            let new_major_status = match error_split(code) {
                ...
                (category, reason) => {
                    ...
                    error!(
                        *log_context,
                        "[aptos_vm] Unexpected prologue Move abort: {:?}::{:?} (Category: {:?} \
                         Reason: {:?})",
                        location,
                        code,
                        category,
...
pub fn convert_epilogue_error(
    ...
    Err(match status {
            ...
            if !transaction_validation.is_account_module_abort(&location) =>
        {
            ...
            error!(
                *log_context,
                "[aptos_vm] Unexpected success epilogue Move abort: {:?}::{:?} (Category: {:?} \
                 Reason: {:?})",
                location,
                code,
                category,
...
pub fn expect_only_successful_execution(
    ...
    Err(match status {
        ...
        status => {
            // Only trigger a warning here as some errors could be a result of the
            // speculative parallel execution. We will report the errors after
            // we obtained the final transaction output in
