...
impl Bullshark {
    ...
    fn find_first_anchor_to_commit(&mut self, mut current_anchor: Node) -> Node {
        let mut anchor_round = current_anchor.round();

        while anchor_round > self.lowest_unordered_anchor_round {
            anchor_round -= 2;
            let prev_anchor_peer_id = self
                .proposer_election
                .get_round_anchor_peer_id(anchor_round);

            if self.dag[anchor_round as usize]
                .get(&prev_anchor_peer_id)
                .map_or(false, |prev_anchor| {
                    self.strong_path(&current_anchor, prev_anchor)
                })
            {
                current_anchor = self.dag[anchor_round as usize]
                    .get(&prev_anchor_peer_id)
                    .unwrap()
                    .clone();
            }
