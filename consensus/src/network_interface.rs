...
pub enum ConsensusMsg {
    ...
    /// protocol to make its choice (typically depends on round and proposer
    /// info).
    ProposalMsg(Box<ProposalMsg>),
    /// This struct describes basic synchronization metadata.
    SyncInfo(Box<SyncInfo>),
    /// A vector of LedgerInfo with contiguous increasing epoch numbers to prove
    /// a sequence of epoch changes from the first LedgerInfo's epoch.
    ...
    VoteMsg(Box<VoteMsg>),
    /// CommitProposal is the struct that is sent by the validator after
    /// execution to propose on the committed state hash root.
    CommitVoteMsg(Box<CommitVote>),
    /// CommitDecision is the struct that is sent by the validator after
    /// collecting no fewer than 2f + 1 signatures on the commit proposal.
    /// This part is not on the critical path, but it can save slow machines
    /// to quickly confirm the execution result.
    CommitDecisionMsg(Box<CommitDecision>),
    /// Quorum Store: Send a fragment -- a sequence of transactions that are
    /// part of an in-progress batch -- from the fragment generator to
    /// remote validators.
    FragmentMsg(Box<Fragment>),
    /// Quorum Store: Request the payloads of a completed batch.
    BatchRequestMsg(Box<BatchRequest>),
    /// Quorum Store: Respond with a completed batch's payload -- a sequence of
    /// transactions, identified by its digest.
    BatchMsg(Box<Batch>),
    /// Quorum Store: Send a signed batch digest. This is a vote for the batch
    /// and a promise that the batch of transactions was received and will
    /// be persisted until batch expiration.
    SignedDigestMsg(Box<SignedDigest>),
    /// Quorum Store: Broadcast a certified proof of store (a digest that
    /// received 2f+1 votes).
...
impl ConsensusNetworkSender {
    /// Initialize a shared hashmap about connections metadata that is updated
    /// by the receiver.
    ...
    /// Choose the overlapping protocol for peer. The local protocols are sorted
...
impl ApplicationNetworkSender<ConsensusMsg> for ConsensusNetworkSender {
    /// Send a single message to the destination peer using the available
    /// ProtocolId.
    ...
    /// Send a single message to the destination peers using the available
    /// ProtocolId.
    fn send_to_many(
        &self,
