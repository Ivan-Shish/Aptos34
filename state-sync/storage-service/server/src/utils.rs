...
pub fn notify_peer_of_new_data<T: StorageReaderInterface>(
    ...
    let use_compression = storage_service_request.use_compression;
    let handler = Handler::new(
        cached_storage_server_summary,
        optimistic_fetches,
        lru_response_cache,
    ...
    let storage_response =
        handler.process_request(peer_network_id, storage_service_request.clone(), true);

    // Transform the missing data into a new data response
