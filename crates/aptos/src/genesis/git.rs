    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            Err(CliError::CommandArgumentError(
                "Invalid repository must be of the form 'owner/repository` e.g. \
...
impl GitOptions {
    pub fn get_client(self) -> CliTypedResult<Client> {
            ...
            && self.local_repository_dir.is_none()
        {
            ...
            Err(CliError::CommandArgumentError(
                "Must provide either only --local-repository-dir or both --github-repository and \
                 --github-token-path"
                    .to_string(),
            ))
        }
...
impl Client {
    ...
    pub fn create_dir(&self, dir: &Path) -> CliTypedResult<()> {
        match self {
            ...
            Client::Github(_) => {
                // There's no such thing as an empty directory in Git, so do
                // nothing
            },
        }
