...
/// Benchmarks the time to verify a signature on an empty message. (Used for gas
/// estimation.)
fn sig_verify_zero_bytes<M: Measurement>(g: &mut BenchmarkGroup<M>) {
    ...
    g.bench_function("sig_verify_zero_bytes", move |b| {
        b.iter_with_setup(
            || {
                // Just want to ensure a different signature each time, so signing a random
                // message each time
                let msg = random_message(&mut csprng);
...
/// Benchmarks the time to deserialize an Ed25519 public key from a sequence of
/// bytes.
...
/// Benchmarks the time to deserialize an Ed25519 signature from a sequence of
/// bytes.
