...
//! Function based on [RFC 5869](https://tools.ietf.org/html/rfc5869).
//!
...
//! recommended approach for generating keys, especially when a True Random
//! Generator is not available. b) derivation of session keys from a shared
//! Diffie-Hellman value in a key-agreement protocol. c) combining entropy from
//! multiple sources of randomness, such as entropy collected from system
//! events, user's keystrokes, /dev/urandom etc. The combined seed can then be
//! used to generate cryptographic keys for account, network and transaction
//! signing keys among the others. d) hierarchical private key derivation,
//! similarly to Bitcoin's BIP32 protocol for easier key management.
...
/// Seed (ikm = initial key material) is not accepted if its size is less than
/// 16 bytes. This is a precautionary measure to prevent HKDF misuse. 128 bits
/// is the minimum accepted seed entropy length in the majority of today's
/// applications to avoid brute forcing. Note that for Ed25519 keys, random
/// seeds of at least 32 bytes are recommended.
    ...
    D::OutputSize: IsGreaterOrEqual<DMinimumSize, Output = True>,
{
    ...
    /// CAUTION: This is not recommended because it does not take an ikm (seed)
    /// as an input and thus, it is not fully compliant with the HKDF RFC
    /// (which always expects a non-zero ikm). Please use
    /// `extract_then_expand` instead, unless you know what you are doing.
    ///
    /// This api is currently required by the Noise protocol [HKDF specs](https://noiseprotocol.org/noise.html#hash-functions).
    ///
    /// HKDF Extract then Expand operation as a single step, but without an ikm
    /// input.
...
pub enum HkdfError {
    ...
    /// bits.
    #[error(
        "HKDF expand error - the pseudorandom key input ('prk' in RFC 5869) is less than the \
         underlying hash output bits"
    )]
