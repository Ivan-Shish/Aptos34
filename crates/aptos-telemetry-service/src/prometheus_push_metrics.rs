...
fn claims_to_extra_labels(claims: &Claims, common_name: Option<&String>) -> Vec<String> {
    ...
    let pod_name = if let Some(common_name) = common_name {
        ...
        // for community nodes we cannot determine which pod name they run in (or
        // whether they run in k8s at all), so we use the peer id as an
        // approximation/replacement for pod_name This works well with our
