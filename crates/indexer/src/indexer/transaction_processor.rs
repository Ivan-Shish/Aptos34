...
/// The `TransactionProcessor` is used by an instance of a `Tailer` to process
/// transactions
#[async_trait]
pub trait TransactionProcessor: Send + Sync + Debug {
    /// name of the processor, for status logging
    /// This will get stored in the database for each (`TransactionProcessor`,
    /// transaction_version) pair
    fn name(&self) -> &'static str;

    /// Process all transactions within a block and processes it. This method
    /// will be called from `process_transaction_with_status`
    /// In case a transaction cannot be processed, we will fail the entire
    /// block.
    ...
    /// Gets the connection.
    /// If it was unable to do so (default timeout: 30s), it will keep retrying
    /// until it can.
    ...
    /// This is a helper method, tying together the other helper methods to
    /// allow tracking status in the DB
    ...
    /// Writes that a version has been started for this `TransactionProcessor`
    /// to the DB
    ...
    /// Writes that a version has been completed successfully for this
    /// `TransactionProcessor` to the DB
    ...
    /// Writes that a version has errored for this `TransactionProcessor` to the
    /// DB
