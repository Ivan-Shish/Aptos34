...
#[derive(QueryId)]
/// Using this will append a where clause at the end of the string upsert
/// function, e.g. INSERT INTO ... ON CONFLICT DO UPDATE SET ... WHERE
/// "transaction_version" = excluded."transaction_version" This is needed when
/// we want to maintain a table with only the latest state
...
/// Given diesel has a limit of how many parameters can be inserted in a single
/// operation (u16::MAX) we may need to chunk an array of items based on how
/// many columns are in the table. This function returns boundaries of chunks in
/// the form of (start_index, end_index)
...
/// This function will clean the data for postgres. Currently it has support for
/// removing null bytes from strings but in the future we will add more
/// functionality.
    ...
    U: QueryFragment<Pg> + diesel::query_builder::QueryId,
{
    ...
    // This is needed because if we don't insert any row, then diesel makes a call
    // like this SELECT 1 FROM TABLE WHERE 1=0
