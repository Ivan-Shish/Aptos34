    ...
    T: Clone + fmt::Debug,
{
    ...
    fn get(&self, at: usize) -> Option<(&T, usize)> {
        // Unlike `RepeatVec`, this actually could return &(T, usize) because that's how
        // data is stored internally. But keeping the signature identical makes
        // more sense.
...
fn basic_ops_impl(repeat_vec: impl RepeatVecMethods<&'static str>) {
    ...
    // Removing multiple elements shifts all other elements to the left. 6 and 7 are
    // ignored as out of bounds. Removing 0 and 1 causes the entire surface to
    // be dropped.
