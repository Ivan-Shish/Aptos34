...
/// security requirements. We recommend that keys be stored in key managers. If
/// we make keys unclonable, then the configs must be mutable and that becomes a
/// requirement strictly as a result of supporting test environments, which is
/// undesirable. Hence this internal wrapper allows for keys to be clonable but
/// only from configs.
