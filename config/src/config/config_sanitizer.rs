...
/// Validate and process the failpoints config according to the node type and chain ID
...
/// Validate and process the fullnode network configs according to the node type and chain ID
...
/// Validate and process the validator network config according to the node type and chain ID
...
mod tests {
    ...
    #[test]
    fn test_sanitize_missing_public_fullnode_network_configs() {
    ...
    #[test]
    fn test_sanitize_missing_validator_fullnode_network_configs() {
        // Create a fullnode config with empty fullnode network configs
        let mut node_config = NodeConfig {
            full_node_networks: vec![],
            ..Default::default()
        ...
        // Sanitize the VFN config and verify that it fails
        let error = sanitize_fullnode_network_configs(
            &mut node_config,
