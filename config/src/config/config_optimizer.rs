...
use crate::{
    config::{
        node_config_loader::NodeType, Error, InspectionServiceConfig, LoggerConfig, NodeConfig,
        StateSyncConfig,
    },
...
pub trait ConfigOptimizer {
    ...
    /// config as provided by the user. This is required to check if a value
...
impl ConfigOptimizer for NodeConfig {
    ...
    fn optimize(
        ...
        InspectionServiceConfig::optimize(node_config, local_config_yaml, node_type, chain_id)?;
        LoggerConfig::optimize(node_config, local_config_yaml, node_type, chain_id)?;
        StateSyncConfig::optimize(node_config, local_config_yaml, node_type, chain_id)?;
        optimize_validator_network_config(node_config, local_config_yaml, node_type, chain_id)?;
...
mod tests {
    ...
    fn test_optimize_validator_network_config() {
        ...
        // Optimize the validator network config
