...
impl ConfigOptimizer for NodeConfig {
    fn optimize(
        ...
        // Log whether or not any config modifications were made
        if optimizers_with_modifications.is_empty() {
            println!("No config modifications were made by the config optimizer.");
            Ok(false)
        } else {
            println!(
                "The following config optimizers modified the node config: {:?}",
                optimizers_with_modifications
            );
            Ok(true)
        }
    }
