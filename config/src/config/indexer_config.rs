...
impl ConfigSanitizer for IndexerConfig {
    fn sanitize(node_config: &mut NodeConfig, _chain_id: ChainId) -> Result<(), Error> {
        ...
        indexer_config.fetch_tasks = default_if_zero(
            indexer_config.indexer.fetch_tasks.map(|v| v as u64),
            DEFAULT_FETCH_TASKS as u64,
        )
        .map(|v| v as u8);
        indexer_config.processor_tasks =
            default_if_zero(indexer_config.processor_tasks, DEFAULT_PROCESSOR_TASKS)
...
fn default_if_zero<T>(value: Option<T>, default: T) -> Option<T> {
    match value {
        None => Some(default),
        Some(0) => Some(default),
